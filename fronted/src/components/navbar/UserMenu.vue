  ...
  <div class="dropdown dropdown-end">
    ...
    <ul tabindex="-1" class="dropdown-content menu bg-base-100 rounded-box z-1 w-48 p-2 shadow-lg">
     <li>
      ...
      <li>
        <RouterLink @click="closeMenu" :to="{name: 'user-space-index',params: {user_id: user.id}}" class='text-sm font-blod py-3'>
          <UserSpaceIcon />
          个人空间
        </RouterLink>
      </li>
      <li>
        <RouterLink @click="closeMenu" :to="{name: 'user-profile-index'}" class='text-sm font-blod py-3'>
          <UserProfileIcon />
          编辑资料
        </RouterLink>
      </li>
      <li></li>
      <li>
        <a @click="handleLogout()" class='text-sm font-blod py-3'>
          <UserLogoutIcon />
          退出登录
        </a>
      </li>
