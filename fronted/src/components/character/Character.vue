...
<template>
  <div>
    <div class="avatar cursor-pointer"  @mouseover="isHover = true" @mouseleave="isHover = false" @click="openChatField()">
      <div class="w-60 h-100 rounded-2xl relative" >
        ...
        <div v-if="canRemoveFriend" class="absolute right-0 top-50">
          <button @click.stop="handleUpdateCharacter()" class="btn btn-circle btn-ghost bg-transparent">
            <RemoveIcon />
          </button>
        </div>
        ...
        <div class="absolute left-24 top-58 text-white font-bold line-clamp-1 break-all">
          {{character.name}}
        </div>
        <div class="absolute left-4 top-72 text-white line-clamp-4 break-all">
          {{character.profile}}
        </div>
